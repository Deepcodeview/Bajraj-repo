...
export function generateCustomerReportPdf(report) {
  return new Promise((resolve, reject) => {
    ...
    doc.fontSize(9).fillColor(GRAY).font("Helvetica")
       .text("Customer Footfall Report", L + 16, 74);
    ...
    const cards = [
      { label: "Total Visitors",   value: report.totalVisitors,              color: BLUE,   bg: LIGHT_BLUE, bd: BLUE_BD   },
      { label: "Active Now",       value: report.active,                     color: GREEN,  bg: GREEN_BG,   bd: GREEN_BD  },
      { label: "Completed",        value: report.completed,                  color: AMBER,  bg: AMBER_BG,   bd: AMBER_BD  },
      { label: "Avg Duration(min)",value: report.avgDurationMin,             color: PURPLE, bg: PURPLE_BG,  bd: PURPLE_BD },
    ];
    ...
    doc.fontSize(10).fillColor(DARK).font("Helvetica-Bold")
       .text("Customer Session Records", L, secY);
    ...
    const cols = [
      ...
      { label: "Session Code", w: 110, align: "left"   },
      { label: "Entry Time",   w: 72,  align: "center" },
      { label: "Exit Time",    w: 72,  align: "center" },
      { label: "Duration",     w: 62,  align: "center" },
      { label: "Zone",         w: 80,  align: "center" },
      { label: "Assigned To",  w: 80,  align: "center" },
      { label: "Status",       w: 58,  align: "center" },
    ];
    ...
    if (report.records.length === 0) {
      ...
      doc.fontSize(9).fillColor(GRAY).font("Helvetica")
         .text("No customer sessions found for this date.", L, rowY + 10, { width: W, align: "center" });
      rowY += 30;
    }
    ...
    report.records.forEach((rec, idx) => {
      ...
      const statusColor = rec.status === "ACTIVE" ? GREEN : rec.status === "COMPLETED" ? AMBER : GRAY;
    ...
    if (rowY + 20 < doc.page.height - 55) {
      ...
      doc.fontSize(8).fillColor(BLUE).font("Helvetica-Bold")
         .text(
           `Total: ${report.totalVisitors}  |  Active: ${report.active}  |  Completed: ${report.completed}  |  Avg Duration: ${report.avgDurationMin} min`,
