...
export function generateAttendancePdf(report) {
  return new Promise((resolve, reject) => {
    ...
    doc.fontSize(52).fillColor("#d1d5db").fillOpacity(0.15)
       .font("Helvetica-Bold")
    ...
    doc.fontSize(9).fillColor(GRAY).font("Helvetica")
       .text("Employee Attendance Report", L + 16, 74);
    ...
    const present    = report.records.filter(r => r.status !== "Absent").length;
    const absent     = report.records.filter(r => r.status === "Absent").length;
    const checkedOut = report.records.filter(r => r.status === "Checked Out").length;

    const cards = [
      { label: "Total Employees", value: report.totalEmployees, color: BLUE,  bg: LIGHT_BLUE, bd: BLUE_BD  },
      { label: "Present",         value: present,               color: GREEN, bg: GREEN_BG,   bd: GREEN_BD },
      { label: "Absent",          value: absent,                color: RED,   bg: RED_BG,     bd: RED_BD   },
      { label: "Checked Out",     value: checkedOut,            color: AMBER, bg: AMBER_BG,   bd: AMBER_BD },
    ];
    ...
    doc.fontSize(10).fillColor(DARK).font("Helvetica-Bold")
       .text("Attendance Records", L, secY);
    ...
    const cols = [
      ...
      { label: "Employee",   w: 135, align: "left"   },
      { label: "Code",       w: 60,  align: "center" },
      { label: "Check In",   w: 68,  align: "center" },
      { label: "Check Out",  w: 68,  align: "center" },
      { label: "Duration",   w: 62,  align: "center" },
      { label: "Status",     w: 61,  align: "center" },
    ];
    ...
    if (report.records.length === 0) {
      ...
      doc.fontSize(9).fillColor(GRAY).font("Helvetica")
         .text("No attendance records found for this date.", L, rowY + 10, { width: W, align: "center" });
      rowY += 30;
    }
    ...
    report.records.forEach((rec, idx) => {
      ...
      const statusColor = rec.status === "Absent"      ? RED
        : rec.status === "Present"                      ? GREEN
        : rec.status === "Checked Out"                  ? AMBER : GRAY;
    ...
    if (rowY + 20 < doc.page.height - 55) {
      ...
      doc.fontSize(8).fillColor(BLUE).font("Helvetica-Bold")
         .text(
           `Total: ${report.records.length}  |  Present: ${present}  |  Absent: ${absent}  |  Checked Out: ${checkedOut}`,
...
function _addFooter(doc, L, W, orgName) {
  ...
  doc.fontSize(7.5).fillColor("#9ca3af").font("Helvetica")
     .text(
       `${orgName}  •  Confidential  •  ${new Date().toLocaleString("en-IN")}`,
